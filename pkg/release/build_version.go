...
func (b *BuildVersionClient) SetBuildVersion(
	...
	if branch == git.DefaultBranch {
		branch = "release-" + git.DefaultBranch
		logrus.Infof("Changing %s branch to %q", git.DefaultBranch, branch)
	}
...
func (g *defaultGcpClient) CopyJobCache(job string) (jsonPath string) {
	...
	if err := gcp.GSUtil("-qm", "cp", remotePath, jsonPath); err != nil {
		logrus.Infof("Skipping unavailable remote path %s", remotePath)
		return ""
	}
	return jsonPath
}
...
// GetJobCache pulls Jenkins server job cache from GS and returns a `JobCache`
//
