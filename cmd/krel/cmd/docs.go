...
e.g A,C,D,E,J where A = kep column, C = website pr column, D = sig column etc`)
	...
	automateCmd.MarkFlagRequired("sheet-id")
	automateCmd.MarkFlagRequired("release")

	rootCmd.AddCommand(automateCmd)
}

func getPrStruct(sheetID string, readRange string, headers []string) ([]*docs.PrData, error) {
	if len(headers) == 4 {
