...
func afterFreezeRun(cobra *cobra.Command, args []string) error {
	...
	for _, issuePr := range issueMaps {
		...
		kubernetesPr, _, err := glClient.PullRequests.Get(ctx, k8sOrg, k8sOrg, issuePr.KubernetesPR)
		if err != nil {
			return err
		}
		...
		if !kubernetesPr.GetMerged() {
			_, _, err := glClient.Issues.AddLabelsToIssue(ctx, k8sOrg, websiteRepo, issuePr.WebsitePR, []string{"do-not-merge/hold"})
			if err != nil {
				return fmt.Errorf("error putting label on pull request: %s", err)
			}
		}

		websitePR, _, err := glClient.PullRequests.Get(ctx, k8sOrg, websiteRepo, issuePr.WebsitePR)
		if err != nil {
			return err
		}
		...
		if websitePR.Milestone.GetNumber() != afterFreezeOpts.milestoneID {
			...
			_, _, err := glClient.Issues.Edit(ctx, k8sOrg, websiteRepo, issuePr.WebsitePR, &issue)
			if err != nil {
				return err
			}
		}
		...
		if websitePR.GetBase().GetRef() != baseBranch {
			...
			err := docs.CommentOnIssue(ctx, glClient, websiteRepo, issuePr.WebsitePR, templates.WrongBaseBranch, msgVar)
			if err != nil {
				return err
			}
		}
