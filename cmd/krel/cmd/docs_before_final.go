...
func beforeFinalRun(cmd *cobra.Command, args []string) error {
	...
	for _, data := range prdata {
		websitePR, _, err := glClient.PullRequests.Get(ctx, k8sOrg, websiteRepo, data.KubernetesPR)
		if err != nil {
			return err
		}
		...
		if websitePR.GetMerged() {
			...
			_, _, err := glClient.Issues.AddLabelsToIssue(ctx, k8sOrg, websiteRepo, data.WebsitePR, []string{})
			if err != nil {
		...
		err = docs.CommentOnIssue(ctx, glClient, websiteRepo, data.WebsitePR, templates.UpcomingDeadline, docs.MessageVar{Name: data.Assignee})
		if err != nil {
			return err
		}
	}
